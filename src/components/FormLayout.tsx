            ...
            {steps.map((step, index) => (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center gap-2 cursor-pointer" onClick={() => setCurrentStep(step.id)}>
                  ...
                  <span
                    className={`text-xs font-medium text-center max-w-20 ${currentStep === step.id
                      ? "text-primary"
                      : currentStep > step.id
                        ? "text-primary/70"
                ...
                {index < steps.length - 1 && (
                  <div
                    className={`flex-1 h-0.5 mx-2 rounded-full transition-colors ${currentStep > step.id ? "bg-primary/50" : "bg-border"
                      }`}
                  />
                )}
              </React.Fragment>
