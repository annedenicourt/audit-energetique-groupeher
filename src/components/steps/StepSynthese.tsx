   ...
   return (
     <div className="space-y-6">
       ...
       <SectionCard title="Habitation" icon={Home}>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
           <SummaryRow label="Année de construction" value={data.habitation.anneeConstruction} />
           <SummaryRow label="Surface habitable" value={data.habitation.surfaceHabitable ? `${data.habitation.surfaceHabitable} m²` : ""} />
           <SummaryRow label="Nombre de personnes" value={data.habitation.nbrePersonnes} />
           <SummaryRow label="Type de chauffage" value={data.habitation.typeChauffage} />
           <SummaryRow label="Eau chaude" value={data.habitation.typeEauChaude} />
           <SummaryRow label="Ventilation" value={data.habitation.typeAeration} />
         </div>
       </SectionCard>
       ...
       <SectionCard title="Factures" icon={Receipt}>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
           <SummaryRow label="Facture électricité annuelle" value={data.factures.factureElecAnnuelle ? `${data.factures.factureElecAnnuelle} €` : ""} />
           <SummaryRow label="Facture énergie annuelle" value={data.factures.factureEnergieAnnuelle ? `${data.factures.factureEnergieAnnuelle} €` : ""} />
           <SummaryRow label="Travaux réalisés" value={data.factures.travauxRealises} />
           <SummaryRow label="Aides perçues" value={data.factures.montantAides ? `${data.factures.montantAides} €` : ""} />
         </div>
       </SectionCard>
       ...
       <SectionCard title="Bilan énergétique" icon={BarChart3}>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
           ...
           <SummaryRow label="Énergie actuelle" value={data.bilan.energieActuelle} />
       ...
       <SectionCard title="Financement" icon={Banknote}>
         <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
           ...
           <SummaryRow label="Économies sur 10 ans" value={data.financement.economiesRealisees10Ans ? `${data.financement.economiesRealisees10Ans} €` : ""} />
         </div>
       </SectionCard>
